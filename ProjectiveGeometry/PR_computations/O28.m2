...
-- quadric/octic forms.  But, modulo the GL(3)x Aut(E) action, it can
-- be brought into a normal form.

A = k[a_1,b_1..b_9,c_1..c_9]
S = A[X,Y]
M = matrix ({{X*(a_1*X+Y), X*(X^7)},
    	     {(X-Y)*(b_1*X+Y), (X-Y)*(b_2*X^7 + b_3*X^6*Y + b_4*X^5*Y^2 + b_5*X^4*Y^3 + b_6*X^3*Y^4 + b_7*X^2*Y^5 + b_8*X*Y^6 + b_9*Y^7)},	
    	     {(X-2*Y)*(c_1*X+Y), (X-2*Y)*(c_2*X^7 + c_3*X^6*Y + c_4*X^5*Y^2 + c_5*X^4*Y^3 + c_6*X^3*Y^4 + c_7*X^2*Y^5 + c_8*X*Y^6 + c_9*Y^7)}})
...
-- The result is a global section of O(8)+O(12), which we have to take modulo Aut.
monsSmaller = flatten entries gens (ideal(X,Y))^10
monsBigger = flatten entries gens (ideal(X,Y))^16
monsDiff = flatten entries gens (ideal(X,Y))^6
...
-- We pick a particular k-valiued point of P8 + P12 modulo P8
kXY = k[X,Y]
imSmaller = random(10,kXY)
imBigger = random(16, kXY)
monsSmaller = flatten entries gens (ideal(X,Y))^10
monsBigger = flatten entries gens (ideal(X,Y))^16
monsDiff = flatten entries gens (ideal(X,Y))^6
