...
-- linear/cubic forms.  But, modulo the GL(3)x Aut(E) action, it can be
-- brought into the following normal form.

--   X 	   [1, 0]
--   (X-Y) [1, b1*X^2 + b2*X*Y + Y^2]
--   (X-2Y)[1, c1*X^2 + c2*X*Y + c3*Y^2]

A = k[b_1..b_2,c_1..c_3]
S = A[X,Y]
M = matrix ({{X*(1), X*(0)},
    	     {(X-Y)*(1), (X-Y)*(b_1*X^2 + b_2*X*Y + Y^2)},	
    	     {(X-2*Y)*(1), (X-2*Y)*(c_1*X^2 + c_2*X*Y + c_3*Y^2)}})
...
-- The result is a global section of O(3)+O(5), which we have to take modulo Aut.
monsSmaller = flatten entries gens (ideal(X,Y))^3
monsBigger = flatten entries gens (ideal(X,Y))^5
...
dMQbigger = {dMQ_0*X^2, dMQ_0*X*Y, dMQ_0*Y^2, dMQ_1}
...
kXY = k[X,Y]
imSmaller = random(3,kXY)
imBigger = random(5, kXY)
monsSmaller = flatten entries gens (ideal(X,Y))^3
monsBigger = flatten entries gens (ideal(X,Y))^5
imS = transpose gens kernel transpose lift((coefficients(imSmaller, Monomials=>monsSmaller))_1, k)
imB = transpose gens kernel lift(matrix {flatten entries (coefficients(imSmaller*X^2, Monomials=>monsBigger))_1,
	                                 flatten entries (coefficients(imSmaller*X*Y, Monomials=>monsBigger))_1,
                                         flatten entries (coefficients(imSmaller*Y^2, Monomials=>monsBigger))_1,
...
firstMinorBigger = det(submatrix(transpose bigger, {0,1, 2, 3}), Strategy => Cofactor);
