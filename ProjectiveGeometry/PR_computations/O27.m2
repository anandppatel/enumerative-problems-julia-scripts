...
-- quadric/septic forms.  But, modulo the GL(3)x Aut(E) action, it can
-- be brought into a normal form.

A = k[a_1,b_1..b_8,c_1..c_8]
S = A[X,Y]
M = matrix ({{X*(a_1*X+Y), X*(X^6)},
    	     {(X-Y)*(b_1*X+Y), (X-Y)*(b_2*X^6 + b_3*X^5*Y + b_4*X^4*Y^2 + b_5*X^3*Y^3 + b_6*X^2*Y^4 + b_7*X*Y^5 + b_8*Y^6)},	
    	     {(X-2*Y)*(c_1*X+Y), (X-2*Y)*(c_2*X^6 + c_3*X^5*Y + c_4*X^4*Y^2 + c_5*X^3*Y^3 + c_6*X^2*Y^4 + c_7*X*Y^5+c_8*Y^6)}})
...
monsBigger = flatten entries gens (ideal(X,Y))^14
monsDiff = flatten entries gens (ideal(X,Y))^5
